    ...
    fn tokens_nonascii() {
        assert_lex(
            ...
            &[
                ...
                (Err(UnknownToken), "l", 18..19),
                (Err(UnknownToken), "À", 19..21),
                (Ok(Words::Cet), "cet", 22..25),
                (Err(UnknownToken), "É", 26..28),
                (Err(UnknownToken), "t", 28..29),
                (Err(UnknownToken), "é", 29..31),
            ],
        )
    }
    ...
    fn tokens_regex_escaped() {
        assert_lex(
            ...
            &[
                ...
                (Err(UnknownToken), "a", 7..8),
                (Err(UnknownToken), "b", 8..9),
                (Err(UnknownToken), "c", 9..10),
                (Err(UnknownToken), "c", 10..11),
                (Err(UnknownToken), "B", 11..12),
                (Err(UnknownToken), "A", 12..13),
            ],
        )
    }
